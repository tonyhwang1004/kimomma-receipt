...
const getReceipts = () => sbFetch("GET", "/receipts?select=*&order=created_at.desc");
const addReceipt = (r) => sbFetch("POST", "/receipts", r);
const deleteReceipt = (id) => sbFetch("DELETE", `/receipts?id=eq.${id}`);
  ...
  return (
    <div style={{
      ...
      <button onClick={onBack} style={{
        ...
        background: C.bg, fontSize: 18, cursor: "pointer",
...
export default function App() {
  ...
  const analyzeReceipt = async (base64, mediaType) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error((await res.json().catch(() => ({}))).error || "서버 오류");
  ...
  const confirmSave = async () => {
    ...
    setLoading(true);
    try {
      const device = navigator.userAgent.includes("Mobile") ? "모바일" : "PC";
      await addReceipt({
        name: preview.name,
    ...
    return (
      <div style={{ minHeight: "100dvh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <div style={{ textAlign: "center", fontFamily: "'Noto Sans KR', sans-serif" }}>
          ...
          <div style={{ fontSize: 18, fontWeight: 600, color: C.text }}>🔍 처리 중...</div>
    ...
    return (
      <div style={{ minHeight: "100dvh", background: C.bg, fontFamily: "'Noto Sans KR', sans-serif" }}>
        ...
        <div style={{ padding: "20px 16px" }}>
          <div style={{ background: C.card, borderRadius: 20, padding: 28, boxShadow: "0 2px 20px rgba(0,0,0,0.06)", border: `1px solid ${C.border}` }}>
            <div style={{ textAlign: "center", marginBottom: 24 }}>
              <div style={{ fontSize: 48, marginBottom: 8 }}>{manualMode && !preview ? "✏️" : "🧾"}</div>
              {!manualMode && (
            ...
            {[
              ...
              <div key={key} style={{ marginBottom: 18 }}>
                ...
                <input
                  type={type || "text"} value={data[key]} placeholder={placeholder}
                  onChange={(e) => {
                  ...
                  style={{
                    ...
                    borderRadius: 12, fontSize: 16, fontFamily: "inherit", color: C.text,
                    background: "#FAFAF7", boxSizing: "border-box", outline: "none",
                  }}
            ...
            <div style={{ display: "flex", gap: 12, marginTop: 28 }}>
              <button onClick={() => { setPreview(null); setManualMode(false); setView("home"); }}
                style={{ flex: 1, padding: "14px", borderRadius: 14, border: `1.5px solid ${C.border}`, background: C.card, fontSize: 15, fontWeight: 600, color: C.textLight, cursor: "pointer", fontFamily: "inherit" }}>취소</button>
              <button onClick={confirmSave}
                style={{ flex: 2, padding: "14px", borderRadius: 14, border: "none", background: `linear-gradient(135deg, ${C.primary}, ${C.primaryDark})`, fontSize: 15, fontWeight: 600, color: "#fff", cursor: "pointer", fontFamily: "inherit", boxShadow: `0 4px 14px ${C.primary}55`, opacity: (!preview || !preview.name) ? 0.5 : 1 }}
                disabled={!preview || !preview.name}>✓ 저장하기</button>
            </div>
          </div>
        </div>
      </div>
    ...
    return (
      <div style={{ minHeight: "100dvh", background: C.bg, fontFamily: "'Noto Sans KR', sans-serif" }}>
        ...
        <div style={{ padding: "12px 16px" }}>
          ...
          {filterName && (
            <div style={{ background: C.successLight, borderRadius: 14, padding: "14px 18px", marginBottom: 14, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontSize: 14, color: C.success, fontWeight: 600 }}>{filterName} 합계</span>
              <span style={{ fontSize: 18, fontWeight: 700, color: C.success }}>{money(filtered.reduce((s, r) => s + r.amount, 0))}</span>
            </div>
          )}
          {filtered.map((r) => (
            <div key={r.id} style={{ background: C.card, borderRadius: 16, padding: "16px 18px", marginBottom: 10, border: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <div style={{ fontSize: 15, fontWeight: 600, color: C.text }}>{r.name}</div>
                <div style={{ fontSize: 12, color: C.textLight, marginTop: 3 }}>
                  {r.date}{r.memo ? ` · ${r.memo}` : ""}{r.device ? ` · ${r.device}` : ""}
                </div>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{ fontSize: 16, fontWeight: 700, color: C.primary }}>{money(r.amount)}</div>
                <button onClick={() => deleteRecord(r.id)} style={{ width: 30, height: 30, borderRadius: 8, border: "none", background: C.dangerLight, color: C.danger, fontSize: 14, cursor: "pointer" }}>✕</button>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div style={{ minHeight: "100dvh", background: C.bg, fontFamily: "'Noto Sans KR', sans-serif" }}>
      <link href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@300;400;500;600;700&display=swap" rel="stylesheet" />
      <div style={{ background: `linear-gradient(135deg, ${C.primary}, ${C.primaryDark})`, padding: "40px 20px 28px", borderRadius: "0 0 32px 32px", boxShadow: `0 6px 24px ${C.primary}33` }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
          ...
          <button onClick={loadFromDB} style={{ marginLeft: "auto", width: 38, height: 38, borderRadius: 12, border: "1.5px solid rgba(255,255,255,0.5)", background: "rgba(255,255,255,0.2)", fontSize: 16, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}
            title="새로고침">🔄</button>
        </div>
        <div style={{ background: "rgba(255,255,255,0.18)", borderRadius: 18, padding: "18px 22px", backdropFilter: "blur(10px)" }}>
          ...
          <div style={{ fontSize: 12, color: "rgba(255,255,255,0.7)", marginTop: 4 }}>영수증 {records.length}건 · ☁️ 클라우드 저장</div>
        </div>
      </div>

      <div style={{ padding: "20px 16px" }}>
        {error && <div style={{ background: C.dangerLight, color: C.danger, padding: "12px 16px", borderRadius: 12, fontSize: 14, marginBottom: 16, fontWeight: 500 }}>{error}</div>}

        <input type="file" accept="image/*" capture="environment" ref={fileRef} onChange={handleFile} style={{ display: "none" }} />
        <button onClick={() => fileRef.current?.click()} style={{ width: "100%", padding: "18px", borderRadius: 18, border: "none", background: `linear-gradient(135deg, ${C.primary}, ${C.primaryDark})`, fontSize: 16, fontWeight: 600, color: "#fff", cursor: "pointer", fontFamily: "inherit", marginBottom: 10, boxShadow: `0 4px 16px ${C.primary}44` }}>📷 영수증 촬영하기</button>

        <div style={{ display: "flex", gap: 10, marginBottom: 24 }}>
          <button onClick={() => { const i = document.createElement("input"); i.type = "file"; i.accept = "image/*"; i.onchange = handleFile; i.click(); }}
            style={{ flex: 1, padding: "14px", borderRadius: 14, border: `1.5px solid ${C.border}`, background: C.card, fontSize: 14, fontWeight: 500, color: C.textLight, cursor: "pointer", fontFamily: "inherit" }}>🖼️ 갤러리</button>
          <button onClick={() => { setManualMode(true); setPreview({ name: "", amount: 0, date: todayStr(), memo: "" }); }}
            style={{ flex: 1, padding: "14px", borderRadius: 14, border: `1.5px solid ${C.border}`, background: C.card, fontSize: 14, fontWeight: 500, color: C.textLight, cursor: "pointer", fontFamily: "inherit" }}>✏️ 수동 입력</button>
        </div>
        ...
        {byName.length > 0 && (
          <>
            <div style={{ fontSize: 15, fontWeight: 700, color: C.text, marginBottom: 14, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span>📊 이름별 합산</span>
              <button onClick={() => setView("history")} style={{ background: "none", border: "none", color: C.primary, fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>전체 내역 →</button>
            </div>
            {byName.map(({ name, total, count }) => (
              <div key={name} onClick={() => { setFilterName(name); setView("history"); }}
                style={{ background: C.card, borderRadius: 16, padding: "16px 20px", marginBottom: 10, border: `1px solid ${C.border}`, cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
        ...
        {records.length === 0 && !syncing && (
          <div style={{ textAlign: "center", padding: "40px 20px", color: C.textLight }}>
            ...
            <div style={{ fontSize: 14, lineHeight: 1.6 }}>어느 기기에서 찍어도<br />모든 기기에서 바로 확인됩니다 ☁️</div>
          </div>
        )}
        ...
        {records.length > 0 && (
          <>
            ...
            {records.slice(0, 5).map((r) => (
              <div key={r.id} style={{ background: C.card, borderRadius: 14, padding: "14px 18px", marginBottom: 8, border: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
        ...
        <div style={{ textAlign: "center", padding: "20px 0 10px", fontSize: 11, color: C.textLight }}>
          김엄마독서실 영수증 관리 v2.0 · Supabase 클라우드 연동
        </div>
      </div>
    </div>
  );
